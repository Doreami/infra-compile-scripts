...
def main():
    ...
    p.add_argument("--cat-column", action="store_true",
                   help="追加受控标量列 `cat`（string，cat = (id-1) % 10 均匀 10 桶），"
                        "供向量+标量混合查询 A/B（标量先行）测试用。默认不加，保持既有表 schema 不变。")
    args = p.parse_args()
    ...
    if args.cat_column:
        # 受控标量列：cat = (id-1) % 10 → 均匀 10 桶，供混合查询 A/B（范围谓词存活率梯度）。
        fields.append({"id": 3, "name": "cat", "type": "string", "required": False})
    ...
    for round_start in range(0, n, outer_step):
        ...
        if args.cat_column:
            cat_arr = pa.array(
                [str((row_id - 1) % 10) for row_id in range(round_start + 1, round_end + 1)],
                type=pa.string())
            batch = pa.table([ids_arr, vec_arr, cat_arr], schema=arrow_schema)
        else:
